...
class Model():
    ...
    def per_cross_validation(self, X_train, X_test, y_train, y_test):
        self.train(X_train, y_train)
        
        self.train_cm = confusion_matrix(y_train, self.model.predict(X_train))
        self.test_cm = confusion_matrix(y_test, self.model.predict(X_test))
    
        return self, self.train_cm, self.test_cm
    ...
    def cross_validate(self, X, y):
        ...
        for train_index, test_index in StratifiedKFold(n_splits=self.cv).split(X, y):
            ...
            if self.multi_threaded:
                t = ThreadWithReturnValue(target = self.per_cross_validation, args=(X_train, X_test, y_train, y_test))
                threads.append(t)
                t.start()
            else:
                svm_model, train_cm, test_cm = self.per_cross_validation(X_train, X_test, y_train, y_test)
        ...
        return svm_models[0], avg_train_cm, avg_test_cm, imp_table, agg_imp_table, std_train_cm, std_test_cm
