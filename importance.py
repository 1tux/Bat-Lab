...
def agg_importance_by_meaning(imp, names):
    ...
    for entity in entities:
        ...
        for paired_agg_feature in paired_agg_features:
            ...
            # ret_d[new_feature_name] = (ret_d[f"BAT_{i}_EF_{paired_agg_feature}"] + ret_d[f"BAT_{j}_EF_{paired_agg_feature}"]) # / 2
...
def plot_f_importance_from_table(table, ax, amount, total_features, shuffled_table):
    ...
    ax.set_yticklabels(ordered_columns_with_cov, horizontalalignment = "left") # ordered_columns -> ordered_columns_with_cov 
