...
def bin_df_and_neuron(df, neuron, bin_size=5):
    e_df = bin_dataset(df, bin_size=5)
    binned_spikes = neuron.groupby(neuron.index.to_series() // 5).apply(lambda x: x.sum())
    ...
    binned_spikes = pd.Series(np.repeat(binned_spikes.values, binned_spikes.replace(0, 1).tolist())).reindex(res_df.index)
...
def cm_to_ba(cm):
    ...
    balanaced_accuracy = weight_ones * correct_ones + \
                        weight_zeros * correct_zeroes
    
    return balanaced_accuracy
...
def feature_engineering(df):
    df[dataset.get_col_name(0, "X^2")] = df[dataset.get_col_name(0, "X")] ** 2 
    df[dataset.get_col_name(0, "Y^2")] = df[dataset.get_col_name(0, "Y")] ** 2 
    df[dataset.get_col_name(0, "HD^2")] = df[dataset.get_col_name(0, "HD")] ** 2 
...
def manual_normalization(df_):
    ...
    for i in dataset.get_bats_names():
        if dataset.get_col_name(i, "X") not in df.columns: continue # BAT doesn't exist
            
        for k in normalization_factors:
            if i == "0" and k in ["D", "A"]: continue
            if i != "0" and k in ["HD"]: continue
