...
def shuffling(neuron, num=1000):
    random.seed(1337)
    # 3 * len(neuron) // 10 # 150K - 100 mins -> 45K 30. 45/150 -> 9/30 -> 3/10
    # the minimum is either 10 minutes, or when not enough datapoints, 10% of them.
    minimum = min(get_minimal_shift(10), len(neuron) // 10)  
    old_idx = set([0])
    idx = 0
